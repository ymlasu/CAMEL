...
data_path = "../data/"
output_path = "../output/curvature effect/"
...
weight_list = np.array([0.0, 0.001, 0.01, 0.2])
...
metrics_curvature_simi=np.zeros([n_monte,n_data,n_weight])
...
for k in range(n_monte):
    ...
    for i in range(n_data):
        ...
        for j in range(n_weight):
            ...
            transformer = CAMEL( w_curv=weight_list[j], w_FP=20, num_iters=400, target_type=target_type, random_state=1)     
...
for i in range(avg_curvature_simi.shape[1]):
    ...
    rects = ax.bar(x + offset, avg_curvature_simi[:,i], width, yerr=std_curvature_simi[:,i],label='curvature weight of ' + str(weight_list[i]))
    # ax.bar_label(rects, padding=3)
    multiplier += 1

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Curvature Similarity Score', fontsize=14)
ax.set_title('Effect of curvature weight factor',fontsize=16)
