...
def data_prep(data_path, dataset='MNIST', size=20000):
    '''
...
data_path = "../data/"
output_path = "../output/metric_learning/"
...
data_compare = ['swiss_roll', 'MNIST']
newdata_ratio = np.array([0.1, 0.1, 0.5, 1.0])
...
n_newdata_ratio=len(newdata_ratio)
...
# Set up the grid
fig = plt.figure(figsize=(8*n_newdata_ratio,6*n_data*n_methods),layout='constrained',dpi=300)
gs = GridSpec(n_data*n_methods, n_newdata_ratio, figure=fig)

digit_axes = np.zeros((n_data*n_methods, n_newdata_ratio), dtype=object)
...
for k in range(n_methods):
    ...
    for i in range(n_data):
        X_total, y_total = data_prep(data_path, data_compare[i], size=20000)
        ...
        if methods_compare[k] == 'PaCMAP':
            transformer = pacmap.PaCMAP(random_state=1)
            X_embedding = transformer.fit_transform(X)
                  
        elif methods_compare[k]  == 'UMAP':
            transformer = umap.UMAP(target_metric=target_metric, target_weight=0.2, random_state=1)
...
plt.savefig(output_path +'model_compare_metric_learning.png')
