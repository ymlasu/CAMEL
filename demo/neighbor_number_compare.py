...
data_path = "../data/"
output_path = "../output/neighbor number/"
...
for k in range(n_monte):
    ...
    for i in range(n_data):
        ...
        for j in range(n_para):
            if methods_compare[k] == 'PaCMAP':
                transformer = pacmap.PaCMAP(n_neighbors=int(para_list[j]))
                X_embedding = transformer.fit_transform(X)
            elif methods_compare[k]  == 'UMAP':
                transformer = umap.UMAP(n_neighbors=int(para_list[j]), target_metric=target_metric,random_state=1)
                ...
                transformer = CAMEL( n_neighbors=int(para_list[j]), target_type=target_type, random_state=1)     
...
for i in range(avg_curvature_simi.shape[1]):
    ...
    rects = ax.bar(x + offset, avg_curvature_simi[:,i], width, yerr=std_curvature_simi[:,i],label='Neighbor Number: ' + str(para_list[i]))
    # ax.bar_label(rects, padding=3)
    multiplier += 1

# Add some text for labels, title and custom x-axis tick labels, etc.
ax.set_ylabel('Curvature Similarity Score', fontsize=14)
ax.set_title('Effect of neighbor numbers ',fontsize=16)
